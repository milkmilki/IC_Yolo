...
- Next prepared option if the search continues:
  ...
  - start conservatively from the stable weighted-CE EMA tau=0.4 constant-lr recipe using `AutoResearch/configs/wm811k_autoresearch_sam.yaml` (`sam_rho: 0.05`, `sam_adaptive: false`, commit `0bbc3d4`); keep validation-only screening and no test metrics, with the new keep threshold `0.9031118086245977`.
