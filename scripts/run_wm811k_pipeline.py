...
def train_yoloctm_model(config: dict[str, Any], run_name: str, resume_checkpoint: Path | None = None) -> Path:
    ...
    argv = [
        ...
        "--lr-schedule",
        str(train_config.get("lr_schedule", "constant")),
        "--onecycle-pct-start",
        str(train_config.get("onecycle_pct_start", 0.3)),
        "--onecycle-div-factor",
        str(train_config.get("onecycle_div_factor", 10.0)),
        "--onecycle-final-div-factor",
        str(train_config.get("onecycle_final_div_factor", 100.0)),
        "--sam-rho",
        str(train_config.get("sam_rho", 0.0)),
        "--sam-adaptive" if bool(train_config.get("sam_adaptive", False)) else "--no-sam-adaptive",
        "--optimizer",
        str(train_config.get("optimizer", "adamw")),
        "--lion-beta1",
        str(train_config.get("lion_beta1", 0.9)),
        "--lion-beta2",
        str(train_config.get("lion_beta2", 0.99)),
        "--weight-decay",
        ...
        "--topology-ring-bins",
        str(ctm_config.get("topology_ring_bins", 4)),
        "--topology-sector-bins",
        str(ctm_config.get("topology_sector_bins", 8)),
        "--topology-hidden-mult",
        str(ctm_config.get("topology_hidden_mult", 1.5)),
        "--ctm-readout",
        str(ctm_config.get("readout", "mean")),
        "--adaptive-steps" if bool(ctm_config.get("adaptive_steps", False)) else "--no-adaptive-steps",
        "--adaptive-min-steps",
        ...
        "--adaptive-halt-policy",
        str(ctm_config.get("adaptive_halt_policy", "confidence")),
        "--learned-halt-threshold",
        str(ctm_config.get("learned_halt_threshold", 0.5)),
        ...
        "--step-supervision-weight",
        str(ctm_config.get("step_supervision_weight", 0.0)),
        "--step-supervision-steps",
        ",".join(str(step) for step in ctm_config.get("step_supervision_steps", [])),
        "--step-consistency-weight",
        str(ctm_config.get("step_consistency_weight", 0.0)),
        "--step-consistency-steps",
        ",".join(str(step) for step in ctm_config.get("step_consistency_steps", [])),
        "--step-consistency-temperature",
        str(ctm_config.get("step_consistency_temperature", 1.0)),
        "--learned-halt-loss-weight",
        ...
        "--ldam-start-epoch",
        str(ctm_config.get("ldam_start_epoch", 7)),
        "--train-sampling",
        ...
        "--progress-batches",
        str(train_config.get("progress_batches", 0)),
        "--device",
        device,
        "--project",
...
def load_yoloctm_checkpoint(checkpoint: Path, device: str):
    ...
    model = YoloCTM(
        ...
        topology_ring_bins=int(args.get("topology_ring_bins", 4)),
        topology_sector_bins=int(args.get("topology_sector_bins", 8)),
        topology_hidden_mult=float(args.get("topology_hidden_mult", 1.5)),
        ...
        adaptive_halt_policy=str(args.get("adaptive_halt_policy", "learned" if any(key.startswith("halt_head.") for key in model_state) else "confidence")),
        learned_halt_threshold=float(args.get("learned_halt_threshold", 0.5)),
